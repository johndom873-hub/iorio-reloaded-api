...
- [ ] Authentication method for the 2 users not yet decided (users table kept deliberately minimal so it doesn't block on this).
- [ ] IV Rank lookback window not yet decided (needs enough `market_data_snapshots` history to compute; industry-standard is a trailing year, could bootstrap shorter with a caveat) — decide when building screener logic.
...
- **users** — `id, email, display_name, created_at`. Minimal on purpose; auth method not yet decided (see open decisions).
...
- **market_data_snapshots** — `id, ticker_id, snapshot_date, implied_volatility, avg_option_volume, captured_at`. Screening-specific data, scoped to the **current shortlist only**.
  - Correction: IV Rank is **not** an IBKR field — IBKR only provides raw implied volatility. IV Rank must be computed by us from accumulated `implied_volatility` history, not stored as a captured value. New shortlist entries will show IV Rank as unavailable until enough history accumulates (lookback window TBD — see open decisions).
