...
//
//   break-even (per share held) = (stock cash paid - stock cash received - net premium) / shares held
// i.e. the price at which the whole cycle's P&L, on the shares still held, is zero. With no
// shares sold this equals "stock cost - net premium / shares acquired" (the approved formula);
// after a partial sale the realized result of the sold shares flows into it, which is the
// consolidated per-cycle view Marcelo asked for ("+$1k in profit for the position").
// Put-only cycle (no shares): the open short put's strike - net premium / put shares.
//
...
export interface OpenCycleSummary {
  sharesHeld: number;
  /** Stock cash paid minus cash received over the cycle, commissions included. */
  stockNetCashOut: number;
  netPremium: number;
...
interface LedgerEvent {
  ...
  cashOut: number; // stock cash paid (+) / received (-)
}
...
export function summarizeOpenCycle(
  ...
  for (const trade of stockTrades) {
    ...
    events.push({ at: trade.at.getTime(), sharesDelta: signedShares, openOptionsDelta: 0, cashOut: trade.side === "buy" ? gross + trade.commission : -(gross - trade.commission) });
  }

  for (const leg of optionLegs) {
    events.push({ at: leg.entryAt.getTime(), sharesDelta: 0, openOptionsDelta: 1, cashOut: 0 });
    ...
    if (isSettledWithoutTrade(leg)) {
      if (leg.expiryClose === null) {
        ...
        if (inTheMoneyBy > 0 && inTheMoneyBy < marginalThreshold) {
          ...
          cashOut = leg.optionType === "put" ? shares * leg.strike : -shares * leg.strike;
        }
      }
    }
    events.push({ at: leg.exitAt.getTime(), sharesDelta, openOptionsDelta: -1, cashOut });
  }
  ...
  const stockNetCashOut = cycleEvents.reduce((sum, event) => sum + event.cashOut, 0);
  ...
  if (unavailableReason === null) {
    if (sharesHeld > 0) breakEvenPerShare = (stockNetCashOut - netPremium) / sharesHeld;
    else if (openShortPutShares === 0) unavailableReason = "no shares or open short put in the cycle";
  }

  return { sharesHeld, stockNetCashOut, netPremium, breakEvenPerShare, unavailableReason, openShortPuts };
}
