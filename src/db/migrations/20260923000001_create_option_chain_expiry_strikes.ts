...
// Replaces per-strike existence probing (option_chain_strike_checks) with the full
// strike grid per expiry, fetched by the nightly capture with one wildcard
// reqContractDetails per expiry and read from here by everything else.
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable("option_chain_expiry_strikes", (table) => {
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.schema.dropTableIfExists("option_chain_expiry_strikes");
}
